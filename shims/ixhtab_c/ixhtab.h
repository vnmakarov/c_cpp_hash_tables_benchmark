...
#define IXHTAB_FORCE_INLINE __attribute__ ((always_inline)) static inline
#endif
...
#define IXHTAB_C_GROUP_SIZE 8
#define IXHTAB_C_EMPTY_H7 0x80
#define IXHTAB_C_DELETED_H7 0xfe
#define IXHTAB_C_ENTRY_DELETED ((ixhtab_ind_t) ~(unsigned) 0)
#define IXHTAB_C_MAX_BIN_SIZE_POWER 15

enum ixhtab_c_action { IXHTAB_C_FIND, IXHTAB_C_INSERT, IXHTAB_C_REPLACE, IXHTAB_C_DELETE };
...
#include <immintrin.h>
#define IXHTAB_C_MASK_SCALE 0
typedef __m128i ixhtab_group_t;

IXHTAB_FORCE_INLINE ixhtab_group_t ixhtab_c_group_load (const unsigned char *p) {
  return _mm_cvtsi64_si128 (*(const long long *) p);
}
IXHTAB_FORCE_INLINE unsigned int ixhtab_c_match_mask (ixhtab_group_t g, unsigned char h7_val) {
  __m128i h7_vec = _mm_set1_epi8 ((char) h7_val);
  return (unsigned int) _mm_movemask_epi8 (_mm_cmpeq_epi8 (g, h7_vec)) & 0xff;
}
IXHTAB_FORCE_INLINE unsigned int ixhtab_c_match_empty (ixhtab_group_t g) {
...
IXHTAB_FORCE_INLINE unsigned int ixhtab_c_match_empty (ixhtab_group_t g) {
  return ixhtab_c_match_mask (g, IXHTAB_C_EMPTY_H7);
}

#else

#define IXHTAB_C_MASK_SCALE 1
#define IXHTAB_C_SWAR_LSB 0x0101010101010101ULL
#define IXHTAB_C_SWAR_MSB 0x8080808080808080ULL
typedef uint64_t ixhtab_group_t;

IXHTAB_FORCE_INLINE ixhtab_group_t ixhtab_c_group_load (const unsigned char *p) {
  return *(const uint64_t *) p;
}
IXHTAB_FORCE_INLINE uint64_t ixhtab_c_match_mask (ixhtab_group_t g, unsigned char h7_val) {
  uint64_t cmp = g ^ (IXHTAB_C_SWAR_LSB * h7_val);
  return (cmp - IXHTAB_C_SWAR_LSB) & ~cmp & IXHTAB_C_SWAR_MSB;
}
IXHTAB_FORCE_INLINE uint64_t ixhtab_c_match_empty (ixhtab_group_t g) { return g & IXHTAB_C_SWAR_MSB; }
...
static inline void ixhtab_get_params (size_t size, size_t *bn, size_t *bp2, size_t *bs) {
  ...
  *bn = 1;
  *bp2 = 0;
  while (*bs >= (1u << IXHTAB_C_MAX_BIN_SIZE_POWER)) {
    *bn *= 2;
    *bs /= 2;
    (*bp2)++;
  }
...
#define DEFINE_IXHTAB(El, Hash, Eq)                                                                        \
  ...
  struct ixhtab_iter_##El {                                                                                \
    ...
    El *ptr;                                                                                               \
                                                                                                           ...
                                                                                                           \
  IXHTAB_FORCE_INLINE void ixhtab_destroy_bin_##El (struct ebin_ixhtab_##El *b) {                          \
    free (b->els);                                                                                         \
    free (b->deleted);                                                                                     \
    free (b->h7);                                                                                          \
    free (b->entries);                                                                                     \
  ...
  static inline ebin_ixhtab_ind_t ixhtab_create_bin_##El (struct ixhtab_##El *t, unsigned int size) {      \
    if (size < IXHTAB_C_GROUP_SIZE) size = IXHTAB_C_GROUP_SIZE;                                                \
    ...
    b->h7 = (unsigned char *) aligned_alloc (IXHTAB_C_GROUP_SIZE, entries_size);                             \
    memset (b->h7, IXHTAB_C_EMPTY_H7, entries_size);                                                         \
    b->entries = (ixhtab_ind_t *) malloc (entries_size * sizeof (ixhtab_ind_t));                           \
    b->groups_mask = entries_size / IXHTAB_C_GROUP_SIZE - 1;                                                 \
    return ind;                                                                                            \
  }                                                                                                        \
                                                                                                           \
                                                                                                           \
  IXHTAB_FORCE_INLINE bool ixhtab_do_1_##El (struct ixhtab_##El *t, struct ebin_ixhtab_##El *bin,          \
                                             ixhtab_hash_t hash, El *el, enum ixhtab_c_action action,        \
                                             El **res) {                                                   \
    unsigned char h7_val = (hash >> (sizeof (size_t) * 8 - 7)) & 0x7f;                                     \
    unsigned int group_ind = (unsigned int) (hash / IXHTAB_C_GROUP_SIZE) & bin->groups_mask;                 \
    unsigned int first_deleted_slot = ~0u;                                                                 \
    for (;;) {                                                                                             \
      unsigned char *group_h7 = bin->h7 + group_ind * IXHTAB_C_GROUP_SIZE;                                   \
      ixhtab_group_t group = ixhtab_c_group_load (group_h7);                                                 \
      uint64_t mmask = (uint64_t) ixhtab_c_match_mask (group, h7_val);                                       \
      while (mmask) {                                                                                      \
        unsigned int bit = __builtin_ctzll (mmask);                                                        \
        if (IXHTAB_C_MASK_SCALE) bit /= 8;                                                                   \
        unsigned int slot = group_ind * IXHTAB_C_GROUP_SIZE + bit;                                           \
        ixhtab_ind_t el_ind = bin->entries[slot];                                                          \
        if (el_ind == IXHTAB_C_ENTRY_DELETED) {                                                              \
          if (first_deleted_slot == ~0u) first_deleted_slot = slot;                                        \
        } else if (Eq (bin->els[el_ind], *el)) {                                                             \
          if (action != IXHTAB_C_DELETE) {                                                                   \
            *res = &bin->els[el_ind];                                                                      \
          } else {                                                                                         \
            t->els_num--;                                                                                  \
            bin->deleted[el_ind / 8] |= 1 << (el_ind % 8);                                                 \
            bin->entries[slot] = IXHTAB_C_ENTRY_DELETED;                                                     \
          }                                                                                                \
          return true;                                                                                     \
        }                                                                                                  \
        mmask &= mmask - 1;                                                                                \
      }                                                                                                    \
      uint64_t emask = (uint64_t) ixhtab_c_match_empty (group);                                              \
      if (emask) {                                                                                         \
        if (action == IXHTAB_C_INSERT || action == IXHTAB_C_REPLACE) {                                         \
          t->els_num++;                                                                                    \
          unsigned int slot;                                                                               \
          if (first_deleted_slot != ~0u) {                                                                 \
            ...
            if (IXHTAB_C_MASK_SCALE) bit /= 8;                                                               \
            slot = group_ind * IXHTAB_C_GROUP_SIZE + bit;                                                    \
          }                                                                                                \
          bin->h7[slot] = h7_val;                                                                          \
  ...
  static inline void ixhtab_split_bin_##El (struct ixhtab_##El *t, ebin_ixhtab_ind_t bin_ind) {            \
    unsigned int size = (t->bins[bin_ind].groups_mask + 1) * IXHTAB_C_GROUP_SIZE / 2;                        \
    ...
    for (;;) {                                                                                             \
      ...
      unsigned int entries_size = (bin->groups_mask + 1) * IXHTAB_C_GROUP_SIZE;                              \
      memset (bin->h7, IXHTAB_C_EMPTY_H7, entries_size);                                                     \
    ...
    memset (ob->deleted, 0, ((ob->groups_mask + 1) * IXHTAB_C_GROUP_SIZE / 2 + 7) / 8);                      \
    struct ebin_ixhtab_##El *nb = &t->bins[new_ind];                                                       \
    memset (nb->deleted, 0, ((nb->groups_mask + 1) * IXHTAB_C_GROUP_SIZE / 2 + 7) / 8);                      \
  }                                                                                                        \
                                                                                                           \
  IXHTAB_FORCE_INLINE void ixhtab_create_##El (struct ixhtab_##El *t, unsigned int min_size) {             \
    unsigned int size;                                                                                     \
    ...
    for (size_t i = 0; i < bn; i++) {                                                                      \
      ...
      t->bins[ind].mask = (ixhtab_hash_t) i;                                                               \
                                                                                                           ...
                                                                                                           \
  IXHTAB_FORCE_INLINE void ixhtab_destroy_##El (struct ixhtab_##El *t) {                                   \
    for (unsigned int i = 0; i < t->bins_num; i++) ixhtab_destroy_bin_##El (&t->bins[i]);                  \
    free (t->bins);                                                                                        \
    free (t->dir);                                                                                         \
  }                                                                                                        \
                                                                                                           \
  IXHTAB_FORCE_INLINE bool ixhtab_perform_##El (struct ixhtab_##El *t, El *el, enum ixhtab_c_action action,  \
                                                El **res) {                                                \
    ...
    if ((action == IXHTAB_C_INSERT || action == IXHTAB_C_REPLACE) && bin->els_bound == els_size) {             \
      bool grow = false;                                                                                   \
      ...
      if (grow && els_size >= (1u << IXHTAB_C_MAX_BIN_SIZE_POWER)) {                                         \
        ...
        for (unsigned int i = start; i < bound; i++) {                                                     \
          ...
          if (hash2 == 0) hash2 = 1;                                                                       \
          El *r;                                                                                           \
          ixhtab_do_1_##El (t, b, hash2, &b->els[i], IXHTAB_C_INSERT, &r);                                   \
          *r = b->els[i];                                                                                  \
        }                                                                                                  \
        t->els_num = saved;                                                                                \
        free (old_deleted);                                                                                \
    ...
    return ixhtab_do_1_##El (t, &t->bins[bin_ind], hash, el, action, res);                                 \
                                                                                                           ...
                                                                                                           \
  IXHTAB_FORCE_INLINE unsigned int ixhtab_els_count_##El (const struct ixhtab_##El *t) {                   \
    return t->els_num;                                                                                     \
  }                                                                                                        \
                                                                                                           \
  IXHTAB_FORCE_INLINE void ixhtab_iter_advance_##El (const struct ixhtab_##El *t,                          \
    ...
    it->ptr = NULL;                                                                                        \
                                                                                                           ...
                                                                                                           \
  IXHTAB_FORCE_INLINE struct ixhtab_iter_##El ixhtab_iter_begin_##El (const struct ixhtab_##El *t) {       \
    struct ixhtab_iter_##El it;                                                                            \
    it.bin_idx = 0;                                                                                        \
    ...
    return it;                                                                                             \
                                                                                                           ...
                                                                                                           \
  IXHTAB_FORCE_INLINE bool ixhtab_iter_valid_##El (const struct ixhtab_iter_##El *it) {                    \
    return it->ptr != NULL;                                                                                \
  }                                                                                                        \
                                                                                                           \
  IXHTAB_FORCE_INLINE void ixhtab_iter_next_##El (const struct ixhtab_##El *t,                             \
                                                  struct ixhtab_iter_##El *it) {                           \
    ++it->el_idx;                                                                                          \
    ixhtab_iter_advance_##El (t, it);                                                                      \
  }
