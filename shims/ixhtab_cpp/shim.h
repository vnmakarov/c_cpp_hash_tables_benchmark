...
template< typename blueprint > struct ixhtab_cpp
{
  ...
  struct hash
  {
    ixht::hash_t operator()( const entry &e ) const
    {
  ...
  using tab = ixht::ixhtab< entry, hash, eq >;
  ...
  static itr_type find( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    bool found = table.perform( temp, ixht::FIND, &res );
  ...
  static void insert( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    entry *res;
    bool found = table.perform( temp, ixht::INSERT, &res );
    if( !found )
      *res = temp;
  }
  ...
  static void erase( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    table.perform( temp, ixht::DELETE, &res );
  }
