// c_cpp_hash_tables_benchmark/shims/ehtab2/shim.h

#include "ehtab2.h"

template< typename blueprint > struct ehtab2
{
  struct entry
  {
  ...
  struct hash
  {
    ehtab2_hash_t operator()( const entry &e ) const
    {
  ...
  using tab = ehtab2_t< entry, hash, eq >;
  ...
  static void advance_itr( table_type &table, itr_type &itr )
  {
    while( itr.bin_idx < table.bins_num )
    {
      ...
      {
        if( bin.h7[itr.el_idx] != EHTAB2_DELETED_H7 )
        {
  ...
  static itr_type find( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    bool found = tab::do_( &table, temp, EHTAB2_FIND, &res );
    itr_type itr;
  ...
  static void insert( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    entry *res;
    bool found = tab::do_( &table, temp, EHTAB2_INSERT, &res );
    if( !found )
      *res = temp;
  }
  ...
  static void erase( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    tab::do_( &table, temp, EHTAB2_DELETE, &res );
  }
...
template<> struct ehtab2< void >
{
  static constexpr const char *label = "ehtab2";
