#ifndef EHTAB_E16_H7_H
#define EHTAB_E16_H7_H
...
typedef uint16_t ehtab_e16_h7_ind_t;
typedef size_t ehtab_e16_h7_hash_t;
typedef unsigned short ehtab_e16_h7_depth_t;
typedef unsigned int ebin_e16_h7_ind_t;

static constexpr ehtab_e16_h7_ind_t EHTAB_E16_H7_EMPTY_IND = 0xfffe;
static constexpr ehtab_e16_h7_ind_t EHTAB_E16_H7_DELETED_IND = 0xffff;
static constexpr unsigned char EHTAB_E16_H7_DELETED_H7 = 0xff;
static constexpr unsigned int EHTAB_E16_H7_MAX_BIN_SIZE_POWER = 15;

static_assert (EHTAB_E16_H7_MAX_BIN_SIZE_POWER <= 15, "bin size must fit in uint16_t entries");

enum ehtab_e16_h7_action { EHTAB_E16_H7_FIND, EHTAB_E16_H7_INSERT, EHTAB_E16_H7_REPLACE, EHTAB_E16_H7_DELETE };

template<typename El>
struct ebin_e16_h7_t {
  ehtab_e16_h7_depth_t depth;
  ehtab_e16_h7_hash_t mask;
  unsigned int els_start, els_bound;
  El *els;
  unsigned char *h7;
  ehtab_e16_h7_ind_t *entries;
  unsigned int entries_mask;
};

template<typename El, typename Hash, typename Eq>
struct ehtab_e16_h7_t {
  unsigned int els_num;
  ehtab_e16_h7_depth_t max_depth;
  ehtab_e16_h7_hash_t bin_mask;
  ...
  static inline void init_entries (ehtab_e16_h7_ind_t *entries, unsigned int count) {
    for (unsigned int i = 0; i < count; i++) entries[i] = EHTAB_E16_H7_EMPTY_IND;
  }

  static ebin_e16_h7_ind_t create_bin (ehtab_e16_h7_t *htab, unsigned int size) {
    ...
    b.entries = (ehtab_e16_h7_ind_t *) std::malloc (entries_size * sizeof (ehtab_e16_h7_ind_t));
  ...
  static void get_params (size_t size, size_t &bins_num, size_t &bin_power2, size_t &bin_size) {
    ...
    while (bin_size >= (1u << EHTAB_E16_H7_MAX_BIN_SIZE_POWER)) {
      bins_num *= 2;
      bin_size /= 2;
      bin_power2++;
    }
  ...
  static void create (ehtab_e16_h7_t *htab, unsigned int min_size) {
    unsigned int size;
    ...
    htab->max_depth = (ehtab_e16_h7_depth_t) bin_power2;
    htab->bin_mask = ((ehtab_e16_h7_hash_t) 1 << bin_power2) - 1;
    ...
    for (size_t i = 0; i < bins_num; i++) {
      ...
      htab->bins[ind].depth = (ehtab_e16_h7_depth_t) bin_power2;
      htab->bins[ind].mask = (ehtab_e16_h7_hash_t) i;
    }
  }

  static void destroy (ehtab_e16_h7_t *htab) {
  ...
  __attribute__((always_inline))
  static inline bool do_1 (ehtab_e16_h7_t *htab, ebin_e16_h7_t<El> &bin, ehtab_e16_h7_hash_t hash, El &el,
			   enum ehtab_e16_h7_action action, El **res) {
    ehtab_e16_h7_ind_t el_ind, *entry, *first_deleted_entry = nullptr;
    unsigned int mask = bin.entries_mask;
    Eq eq_fn;
    unsigned char h7_val = (hash >> (sizeof (size_t) * 8 - 7)) & 0x7f;
    ehtab_e16_h7_hash_t peterb = hash;

    unsigned int ind = hash & mask;
    for (;;) {
      entry = bin.entries + ind;
      el_ind = *entry;
      if (el_ind < EHTAB_E16_H7_EMPTY_IND) {
        if (bin.h7[el_ind] == h7_val && eq_fn (bin.els[el_ind], el)) {
          if (action != EHTAB_E16_H7_DELETE) {
            *res = &bin.els[el_ind];
          } else {
            htab->els_num--;
            bin.h7[el_ind] = EHTAB_E16_H7_DELETED_H7;
            *entry = EHTAB_E16_H7_DELETED_IND;
          }
          return true;
        }
      } else if (el_ind != EHTAB_E16_H7_DELETED_IND) {
        if (action == EHTAB_E16_H7_INSERT || action == EHTAB_E16_H7_REPLACE) {
          htab->els_num++;
  ...
  static void split_bin (ehtab_e16_h7_t *htab, ebin_e16_h7_ind_t bin_ind) {
    ...
    for (;;) {
      ...
      ehtab_e16_h7_hash_t split_mask = (ehtab_e16_h7_hash_t) 1 << bin.depth;
      new_bin.depth = ++bin.depth;
      if (bin.depth > htab->max_depth) {
        htab->max_depth = bin.depth;
        htab->bin_mask = ~(~(ehtab_e16_h7_hash_t) 0 << htab->max_depth);
      ...
      for (unsigned int i = start; i < bound; i++) {
        if (bin.h7[i] == EHTAB_E16_H7_DELETED_H7) continue;
        ehtab_e16_h7_hash_t hash = hash_fn (bin.els[i]);
        if (hash == 0) hash = 1;
        ...
        do_1 (htab, is_old ? htab->bins[bin_ind] : htab->bins[new_ind],
              hash, bin.els[i], EHTAB_E16_H7_INSERT, &r);
        *r = bin.els[i];
      }
  ...
  __attribute__((always_inline))
  static bool do_ (ehtab_e16_h7_t *htab, El &el, enum ehtab_e16_h7_action action, El **res) {
    Hash hash_fn;
    ehtab_e16_h7_hash_t hash = hash_fn (el);
    if (hash == 0) hash = 1;
    ehtab_e16_h7_hash_t dir_ind = hash & htab->bin_mask;
    ...
    if ((action == EHTAB_E16_H7_INSERT || action == EHTAB_E16_H7_REPLACE) && bin.els_bound == els_size) {
      bool grow = false;
      ...
      if (grow && els_size >= (1u << EHTAB_E16_H7_MAX_BIN_SIZE_POWER)) {
        ...
        b.entries = (ehtab_e16_h7_ind_t *) std::realloc (b.entries,
                                                    entries_size * sizeof (ehtab_e16_h7_ind_t));
        b.entries_mask = entries_size - 1;
        init_entries (b.entries, entries_size);
        ...
        htab->els_num = 0;
        for (unsigned int i = start; i < bound; i++) {
          if (b.h7[i] != EHTAB_E16_H7_DELETED_H7) {
            ehtab_e16_h7_hash_t hash2 = hash_fn (b.els[i]);
            if (hash2 == 0) hash2 = 1;
            El *r;
            do_1 (htab, b, hash2, b.els[i], EHTAB_E16_H7_INSERT, &r);
            *r = b.els[i];
          }
        }
      }
