// c_cpp_hash_tables_benchmark/shims/eht_e16_h7/shim.h

#include "eht_e16_h7.h"

template< typename blueprint > struct eht_e16_h7
{
  struct entry
  {
  ...
  struct hash
  {
    eht_e16_h7_hash_t operator()( const entry &e ) const
    {
  ...
  using tab = eht_e16_h7_t< entry, hash, eq >;
  ...
  static void advance_itr( table_type &table, itr_type &itr )
  {
    while( itr.bin_idx < table.bins_num )
    {
      ...
      {
        if( bin.h7[itr.el_idx] != EHT_E16_H7_DELETED_H7 )
        {
  ...
  static itr_type find( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    bool found = tab::do_( &table, temp, EHT_E16_H7_FIND, &res );
    itr_type itr;
  ...
  static void insert( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    entry *res;
    bool found = tab::do_( &table, temp, EHT_E16_H7_INSERT, &res );
    if( !found )
      *res = temp;
  }
  ...
  static void erase( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    tab::do_( &table, temp, EHT_E16_H7_DELETE, &res );
  }
...
template<> struct eht_e16_h7< void >
{
  static constexpr const char *label = "eht_e16_h7";
