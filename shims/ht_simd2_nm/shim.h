// c_cpp_hash_tables_benchmark/shims/ht_simd2_nm/shim.h

#include "ht_simd2_nm.h"

template< typename blueprint > struct ht_simd2_nm
{
  struct entry
  {
  ...
  struct hash
  {
    ht_simd2_nm_hash_t operator()( const entry &e ) const
    {
  ...
  using tab = ht_simd2_nm_t< entry, hash, eq >;
  using table_type = tab;

  struct itr_type
  {
    ht_simd2_nm_size_t el_idx;
    entry *entry_ptr;
  };
  ...
  static itr_type find( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    bool found = tab::do_( &table, temp, HT_SIMD2_NM_FIND, &res );
    itr_type itr;
  ...
  static void insert( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    entry *res;
    bool found = tab::do_( &table, temp, HT_SIMD2_NM_INSERT, &res );
    if( !found )
      *res = temp;
  }
  ...
  static void erase( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    tab::do_( &table, temp, HT_SIMD2_NM_DELETE, &res );
  }
...
template<> struct ht_simd2_nm< void >
{
  static constexpr const char *label = "ht_simd2_nm";
