// c_cpp_hash_tables_benchmark/shims/ihtab_hybr/shim.h

#include "ihtab_hybr.h"

template< typename blueprint > struct ihtab_hybr
{
  struct entry
  {
  ...
  struct hash
  {
    ihtab_hybr_hash_t operator()( const entry &e ) const
    {
  ...
  using tab = ihtab_hybr_t< entry, hash, eq >;
  ...
  static itr_type find( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    bool found = tab::do_( &table, temp, IHTAB_HYBR_FIND, &res );
    itr_type itr;
  ...
  static void insert( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    entry *res;
    bool found = tab::do_( &table, temp, IHTAB_HYBR_INSERT, &res );
    if( !found )
      *res = temp;
  }
  ...
  static void erase( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    tab::do_( &table, temp, IHTAB_HYBR_DELETE, &res );
  }
...
template<> struct ihtab_hybr< void >
{
  static constexpr const char *label = "ihtab_hybr";
