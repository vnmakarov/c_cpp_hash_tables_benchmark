// c_cpp_hash_tables_benchmark/shims/htab3/shim.h

#include "htab3.h"

template< typename blueprint > struct htab3
{
  struct entry
  {
  ...
  struct hash
  {
    htab3_hash_t operator()( const entry &e ) const
    {
  ...
  using tab = htab3_t< entry, hash, eq >;
  ...
  static itr_type find( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    bool found = tab::do_( &table, temp, HTAB3_FIND, &res );
    itr_type itr;
  ...
  static void insert( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    entry *res;
    bool found = tab::do_( &table, temp, HTAB3_INSERT, &res );
    if( !found )
      *res = temp;
  }
  ...
  static void erase( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    tab::do_( &table, temp, HTAB3_DELETE, &res );
  }
...
template<> struct htab3< void >
{
  static constexpr const char *label = "htab3";
