...
#define IHTAB_C_GROUP_SIZE 8
#define IHTAB_C_EMPTY_H7 0x80
#define IHTAB_C_DELETED_H7 0xfe
#define IHTAB_C_ENTRY_DELETED (~(ihtab_ind_t) 0)
#define IHTAB_C_LF_FACTOR 1
#define IHTAB_C_LF_DIVISOR 2

enum ihtab_c_action { IHTAB_C_FIND, IHTAB_C_INSERT, IHTAB_C_REPLACE, IHTAB_C_DELETE };
...
#include <immintrin.h>
#define IHTAB_C_MASK_SCALE 0
typedef __m128i ihtab_group_t;

IHTAB_FORCE_INLINE ihtab_group_t ihtab_c_group_load (const unsigned char *p) {
  return _mm_cvtsi64_si128 (*(const long long *) p);
}
IHTAB_FORCE_INLINE uint64_t ihtab_c_match_mask (ihtab_group_t g, unsigned char h7_val) {
  __m128i h7_vec = _mm_set1_epi8 ((char) h7_val);
  return (uint64_t) _mm_movemask_epi8 (_mm_cmpeq_epi8 (g, h7_vec)) & 0xff;
}
IHTAB_FORCE_INLINE uint64_t ihtab_c_match_empty (ihtab_group_t g) {
...
IHTAB_FORCE_INLINE uint64_t ihtab_c_match_empty (ihtab_group_t g) {
  return ihtab_c_match_mask (g, IHTAB_C_EMPTY_H7);
}

#else

#define IHTAB_C_MASK_SCALE 1
#define IHTAB_C_SWAR_LSB 0x0101010101010101ULL
#define IHTAB_C_SWAR_MSB 0x8080808080808080ULL
typedef uint64_t ihtab_group_t;

IHTAB_FORCE_INLINE ihtab_group_t ihtab_c_group_load (const unsigned char *p) { return *(const uint64_t *) p; }
IHTAB_FORCE_INLINE uint64_t ihtab_c_match_mask (ihtab_group_t g, unsigned char h7_val) {
  uint64_t cmp = g ^ (IHTAB_C_SWAR_LSB * h7_val);
  return (cmp - IHTAB_C_SWAR_LSB) & ~cmp & IHTAB_C_SWAR_MSB;
}
IHTAB_FORCE_INLINE uint64_t ihtab_c_match_empty (ihtab_group_t g) { return g & IHTAB_C_SWAR_MSB; }
...
#define DEFINE_IHTAB(El, Hash, Eq)                                                                           \
  ...
  IHTAB_FORCE_INLINE bool ihtab_do_1_##El (struct ihtab_##El *t, struct hbin_ihtab_##El *b, El *el,          \
                                           enum ihtab_c_action action, El **res) {                             \
    ihtab_hash_t hash = Hash (*el);                                                                          \
    unsigned char h7_val = (hash >> (sizeof (size_t) * 8 - 7)) & 0x7f;                                       \
    ihtab_size_t group_ind = (hash / IHTAB_C_GROUP_SIZE) & b->groups_mask;                                     \
    ihtab_size_t first_deleted_slot = ~(ihtab_size_t) 0;                                                     \
    for (;;) {                                                                                               \
      unsigned char *group_h7 = b->h7 + group_ind * IHTAB_C_GROUP_SIZE;                                        \
      ihtab_group_t group = ihtab_c_group_load (group_h7);                                                     \
      uint64_t mmask = ihtab_c_match_mask (group, h7_val);                                                     \
      while (mmask) {                                                                                        \
        unsigned int bit = __builtin_ctzll (mmask);                                                          \
        if (IHTAB_C_MASK_SCALE) bit /= 8;                                                                      \
        ihtab_size_t slot = group_ind * IHTAB_C_GROUP_SIZE + bit;                                              \
        ihtab_ind_t el_ind = b->entries[slot];                                                               \
        if (el_ind == IHTAB_C_ENTRY_DELETED) {                                                                 \
          if (first_deleted_slot == ~(ihtab_size_t) 0) first_deleted_slot = slot;                            \
        } else if (Eq (b->els[el_ind], *el)) {                                                               \
          if (action != IHTAB_C_DELETE) {                                                                      \
            *res = &b->els[el_ind];                                                                          \
          } else {                                                                                           \
            t->els_num--;                                                                                    \
            b->deleted[el_ind / 8] |= 1 << (el_ind % 8);                                                     \
            b->entries[slot] = IHTAB_C_ENTRY_DELETED;                                                          \
          }                                                                                                  \
          return true;                                                                                       \
        }                                                                                                    \
        mmask &= mmask - 1;                                                                                  \
      }                                                                                                      \
      uint64_t emask = ihtab_c_match_empty (group);                                                            \
      if (emask) {                                                                                           \
        if (action == IHTAB_C_INSERT || action == IHTAB_C_REPLACE) {                                             \
          t->els_num++;                                                                                      \
          ihtab_size_t slot;                                                                                 \
          if (first_deleted_slot != ~(ihtab_size_t) 0) {                                                     \
            ...
            if (IHTAB_C_MASK_SCALE) bit /= 8;                                                                  \
            slot = group_ind * IHTAB_C_GROUP_SIZE + bit;                                                       \
          }                                                                                                  \
          b->h7[slot] = h7_val;                                                                              \
  ...
  static inline void ihtab_rebuild_##El (struct ihtab_##El *t) {                                             \
    ihtab_size_t entries_size = (t->bin.groups_mask + 1) * IHTAB_C_GROUP_SIZE;                                 \
    ihtab_size_t els_size = entries_size * IHTAB_C_LF_FACTOR / IHTAB_C_LF_DIVISOR;                               \
    if (2 * IHTAB_C_LF_DIVISOR * t->els_num >= IHTAB_C_LF_FACTOR * entries_size) {                               \
      entries_size *= 2;                                                                                     \
      els_size = entries_size * IHTAB_C_LF_FACTOR / IHTAB_C_LF_DIVISOR;                                          \
    }                                                                                                        \
    ...
    rb.h7 = (unsigned char *) aligned_alloc (IHTAB_C_GROUP_SIZE, entries_size);                                \
    memset (rb.h7, IHTAB_C_EMPTY_H7, entries_size);                                                            \
    rb.entries = (ihtab_ind_t *) malloc (entries_size * sizeof (ihtab_ind_t));                               \
    rb.groups_mask = entries_size / IHTAB_C_GROUP_SIZE - 1;                                                    \
    rb.els_bound = 0;                                                                                        \
    ...
    for (ihtab_size_t i = 0; i < bound; i++)                                                                 \
      if (!(t->bin.deleted[i / 8] & (1 << (i % 8)))) {                                                       \
        El *r;                                                                                               \
        ihtab_do_1_##El (t, &rb, &t->bin.els[i], IHTAB_C_INSERT, &r);                                          \
        *r = t->bin.els[i];                                                                                  \
      }                                                                                                      \
    t->els_num = saved;                                                                                      \
  ...
  IHTAB_FORCE_INLINE void ihtab_create_##El (struct ihtab_##El *t, ihtab_size_t min_size) {                  \
    ihtab_size_t entries_size = IHTAB_C_GROUP_SIZE;                                                            \
    while (entries_size * IHTAB_C_LF_FACTOR / IHTAB_C_LF_DIVISOR < min_size) entries_size *= 2;                  \
    ihtab_size_t els_size = entries_size * IHTAB_C_LF_FACTOR / IHTAB_C_LF_DIVISOR;                               \
    t->els_num = 0;                                                                                          \
    t->bin.els_bound = 0;                                                                                    \
    ...
    t->bin.h7 = (unsigned char *) aligned_alloc (IHTAB_C_GROUP_SIZE, entries_size);                            \
    memset (t->bin.h7, IHTAB_C_EMPTY_H7, entries_size);                                                        \
    t->bin.entries = (ihtab_ind_t *) malloc (entries_size * sizeof (ihtab_ind_t));                           \
    t->bin.groups_mask = entries_size / IHTAB_C_GROUP_SIZE - 1;                                                \
  }                                                                                                          \
                                                                                                             \
  IHTAB_FORCE_INLINE void ihtab_destroy_##El (struct ihtab_##El *t) { ihtab_destroy_bin_##El (&t->bin); }    \
                                                                                                             \
  IHTAB_FORCE_INLINE bool ihtab_perform_##El (struct ihtab_##El *t, El *el, enum ihtab_c_action action,        \
                                              El **res) {                                                    \
    ihtab_size_t entries_size = (t->bin.groups_mask + 1) * IHTAB_C_GROUP_SIZE;                                 \
    ihtab_size_t els_size = entries_size * IHTAB_C_LF_FACTOR / IHTAB_C_LF_DIVISOR;                               \
    if (action != IHTAB_C_DELETE && __builtin_expect (t->bin.els_bound >= els_size, 0))                        \
      ihtab_rebuild_##El (t);                                                                                \
  ...
  IHTAB_FORCE_INLINE ihtab_size_t ihtab_size_##El (const struct ihtab_##El *t) {                             \
    return (t->bin.groups_mask + 1) * IHTAB_C_GROUP_SIZE * IHTAB_C_LF_FACTOR / IHTAB_C_LF_DIVISOR;                 \
  }                                                                                                          \
                                                                                                             \
