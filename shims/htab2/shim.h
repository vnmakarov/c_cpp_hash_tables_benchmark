// c_cpp_hash_tables_benchmark/shims/htab2/shim.h

#include "htab2.h"

template< typename blueprint > struct htab2
{
  struct entry
  {
  ...
  struct hash
  {
    htab2_hash_t operator()( const entry &e ) const
    {
  ...
  using tab = htab2_t< entry, hash, eq >;
  ...
  static void advance_itr( table_type &table, itr_type &itr )
  {
    ...
    {
      if( table.bin.h7[itr.el_idx] != HTAB2_DELETED_H7 )
      {
  ...
  static itr_type find( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    bool found = tab::do_( &table, temp, HTAB2_FIND, &res );
    itr_type itr;
  ...
  static void insert( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    entry *res;
    bool found = tab::do_( &table, temp, HTAB2_INSERT, &res );
    if( !found )
      *res = temp;
  }
  ...
  static void erase( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    tab::do_( &table, temp, HTAB2_DELETE, &res );
  }
...
template<> struct htab2< void >
{
  static constexpr const char *label = "htab2";
