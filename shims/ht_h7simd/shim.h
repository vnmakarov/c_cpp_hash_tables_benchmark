// c_cpp_hash_tables_benchmark/shims/ht_h7simd/shim.h

#include "ht_h7simd.h"

template< typename blueprint > struct ht_h7simd
{
  struct entry
  {
  ...
  struct hash
  {
    ht_h7simd_hash_t operator()( const entry &e ) const
    {
  ...
  using tab = ht_h7simd_t< entry, hash, eq >;
  using table_type = tab;

  struct itr_type
  {
    ht_h7simd_size_t el_idx;
    entry *entry_ptr;
  };
  ...
  static itr_type find( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    bool found = tab::do_( &table, temp, HT_H7SIMD_FIND, &res );
    itr_type itr;
  ...
  static void insert( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    entry *res;
    bool found = tab::do_( &table, temp, HT_H7SIMD_INSERT, &res );
    if( !found )
      *res = temp;
  }
  ...
  static void erase( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    tab::do_( &table, temp, HT_H7SIMD_DELETE, &res );
  }
...
template<> struct ht_h7simd< void >
{
  static constexpr const char *label = "ht_h7simd";
