...
#include <immintrin.h>
#define HT_H7SIMD_USE_SSE2 1
static constexpr unsigned int HT_H7SIMD_GROUP_SIZE = 16;
#elif defined(__aarch64__) || defined(_M_ARM64)
#include <arm_neon.h>
#define HT_H7SIMD_USE_NEON 1
static constexpr unsigned int HT_H7SIMD_GROUP_SIZE = 8;
#else
#error "ht_h7simd requires SSE2 or NEON"
#endif
...
struct ht_h7simd_group {
  unsigned int match_mask;
  unsigned int empty_mask;
};

__attribute__((always_inline))
static inline ht_h7simd_group ht_h7simd_probe_group (const unsigned char *group_h7,
                                                      unsigned char h7_val) {
#if HT_H7SIMD_USE_SSE2
  ...
  return {
    (unsigned int) _mm_movemask_epi8 (_mm_cmpeq_epi8 (group, h7_vec)),
    (unsigned int) _mm_movemask_epi8 (_mm_cmpeq_epi8 (group, empty_vec))
  };
#elif HT_H7SIMD_USE_NEON
  uint8x8_t group = vld1_u8 (group_h7);
  uint8x8_t match_eq = vceq_u8 (group, vdup_n_u8 (h7_val));
  uint8x8_t empty_eq = vceq_u8 (group, vdup_n_u8 (HT_H7SIMD_EMPTY_H7));
  static const uint8x8_t bit_mask = {1, 2, 4, 8, 16, 32, 64, 128};
  return {
    (unsigned int) vaddv_u8 (vand_u8 (match_eq, bit_mask)),
    (unsigned int) vaddv_u8 (vand_u8 (empty_eq, bit_mask))
  };
#endif
}

__attribute__((always_inline))
static inline unsigned int ht_h7simd_deleted_mask (const unsigned char *group_h7) {
#if HT_H7SIMD_USE_SSE2
  __m128i group = _mm_load_si128 ((const __m128i *) group_h7);
  __m128i del_vec = _mm_set1_epi8 ((char) HT_H7SIMD_DELETED_H7);
  return (unsigned int) _mm_movemask_epi8 (_mm_cmpeq_epi8 (group, del_vec));
#elif HT_H7SIMD_USE_NEON
  uint8x8_t group = vld1_u8 (group_h7);
  uint8x8_t del_eq = vceq_u8 (group, vdup_n_u8 (HT_H7SIMD_DELETED_H7));
  static const uint8x8_t bit_mask = {1, 2, 4, 8, 16, 32, 64, 128};
...
struct ht_h7simd_t {
  ...
  static void create (ht_h7simd_t *htab, ht_h7simd_size_t min_size) {
    ...
    b.h7 = (unsigned char *) std::aligned_alloc (HT_H7SIMD_GROUP_SIZE, entries_size);
  ...
  static bool do_1 (ht_h7simd_t *htab, hbin_h7simd_t<El> &bin, El &el,
    ...
    for (;;) {
      ...
      ht_h7simd_group g = ht_h7simd_probe_group (group_h7, h7_val);

      unsigned int match_mask = g.match_mask;
      while (match_mask) {
      ...
      if (g.empty_mask) {
        if (action == HT_H7SIMD_INSERT || action == HT_H7SIMD_REPLACE) {
          ...
          if (first_deleted_slot != ~(ht_h7simd_size_t) 0) {
            ...
            unsigned int bit = __builtin_ctz (g.empty_mask);
      ...
      if (first_deleted_slot == ~(ht_h7simd_size_t) 0) {
        unsigned int del_mask = ht_h7simd_deleted_mask (group_h7);
        if (del_mask) {
  ...
  static bool do_ (ht_h7simd_t *htab, El &el, enum ht_h7simd_action action, El **res) {
    ...
    if (action != HT_H7SIMD_DELETE && htab->bin.els_bound >= els_size) {
      ...
      resize_bin.h7 = (unsigned char *) std::aligned_alloc (HT_H7SIMD_GROUP_SIZE, entries_size);
