// c_cpp_hash_tables_benchmark/shims/ht_e25h7l_del/shim.h

#include "ht_e25h7l_del.h"

template< typename blueprint > struct ht_e25h7l_del
{
  struct entry
  {
  ...
  struct hash
  {
    ht_e25h7l_del_hash_t operator()( const entry &e ) const
    {
  ...
  using tab = ht_e25h7l_del_t< entry, hash, eq >;
  using table_type = tab;

  struct itr_type
  {
    ht_e25h7l_del_size_t el_idx;
    entry *entry_ptr;
  };
  ...
  static itr_type find( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    bool found = tab::do_( &table, temp, HT_E25H7L_DEL_FIND, &res );
    itr_type itr;
  ...
  static void insert( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    entry *res;
    bool found = tab::do_( &table, temp, HT_E25H7L_DEL_INSERT, &res );
    if( !found )
      *res = temp;
  }
  ...
  static void erase( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    tab::do_( &table, temp, HT_E25H7L_DEL_DELETE, &res );
  }
...
template<> struct ht_e25h7l_del< void >
{
  static constexpr const char *label = "ht_e25h7l_del";
  static constexpr const char *color = "rgb( 220, 160, 50 )";
