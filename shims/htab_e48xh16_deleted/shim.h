// c_cpp_hash_tables_benchmark/shims/htab_e48xh16_deleted/shim.h

#include "htab_e48xh16_deleted.h"

template< typename blueprint > struct htab_e48xh16_deleted
{
  struct entry
  {
  ...
  struct hash
  {
    htab_e48xh16_deleted_hash_t operator()( const entry &e ) const
    {
  ...
  using tab = htab_e48xh16_deleted_t< entry, hash, eq >;
  using table_type = tab;

  struct itr_type
  {
    htab_e48xh16_deleted_size_t el_idx;
    entry *entry_ptr;
  };
  ...
  static itr_type find( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    bool found = tab::do_( &table, temp, HTAB_E48XH16_DELETED_FIND, &res );
    itr_type itr;
  ...
  static void insert( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    entry *res;
    bool found = tab::do_( &table, temp, HTAB_E48XH16_DELETED_INSERT, &res );
    if( !found )
      *res = temp;
  }
  ...
  static void erase( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    tab::do_( &table, temp, HTAB_E48XH16_DELETED_DELETE, &res );
  }
...
template<> struct htab_e48xh16_deleted< void >
{
  static constexpr const char *label = "htab_e48xh16_deleted";
