// c_cpp_hash_tables_benchmark/shims/htab4/shim.h

#include "htab4.h"

template< typename blueprint > struct htab4
{
  struct entry
  {
  ...
  struct hash
  {
    htab4_hash_t operator()( const entry &e ) const
    {
  ...
  using tab = htab4_t< entry, hash, eq >;
  ...
  static void advance_itr( table_type &table, itr_type &itr )
  {
    ...
    {
      if( table.bin.h7[itr.el_idx] != HTAB4_DELETED_H7 )
      {
  ...
  static itr_type find( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    bool found = tab::do_( &table, temp, HTAB4_FIND, &res );
    itr_type itr;
  ...
  static void insert( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    entry *res;
    bool found = tab::do_( &table, temp, HTAB4_INSERT, &res );
    if( !found )
      *res = temp;
  }
  ...
  static void erase( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    tab::do_( &table, temp, HTAB4_DELETE, &res );
  }
...
template<> struct htab4< void >
{
  static constexpr const char *label = "htab4";
