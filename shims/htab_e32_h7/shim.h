// c_cpp_hash_tables_benchmark/shims/htab_e32_h7/shim.h

#include "htab_e32_h7.h"

template< typename blueprint > struct htab_e32_h7
{
  struct entry
  {
  ...
  struct hash
  {
    htab_e32_h7_hash_t operator()( const entry &e ) const
    {
  ...
  using tab = htab_e32_h7_t< entry, hash, eq >;
  using table_type = tab;

  struct itr_type
  {
    htab_e32_h7_size_t el_idx;
    entry *entry_ptr;
  };
  ...
  static void advance_itr( table_type &table, itr_type &itr )
  {
    ...
    {
      if( table.bin.h7[itr.el_idx] != HTAB_E32_H7_DELETED_H7 )
      {
  ...
  static itr_type find( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    bool found = tab::do_( &table, temp, HTAB_E32_H7_FIND, &res );
    itr_type itr;
  ...
  static void insert( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    entry *res;
    bool found = tab::do_( &table, temp, HTAB_E32_H7_INSERT, &res );
    if( !found )
      *res = temp;
  }
  ...
  static void erase( table_type &table, const typename blueprint::key_type &key )
  {
    ...
    temp.key = key;
    entry *res;
    tab::do_( &table, temp, HTAB_E32_H7_DELETE, &res );
  }
...
template<> struct htab_e32_h7< void >
{
  static constexpr const char *label = "htab_e32_h7";
